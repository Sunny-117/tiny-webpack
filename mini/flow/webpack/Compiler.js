...
class Compiler {
  ...
  run(callback) {
    ...
    for (let entryName in entry) {
      ...
      console.log(this.chunks, '多入口 就会有多chunk：this.chunks')
    ...
    console.log(this.assets, 'emit钩子执行前')
    this.hooks.emit.call();
    console.log(this.assets, 'emit钩子执行后')
    //8.在确定好输出内容后，根据配置确定输出的路径和文件名，把文件内容写入到文件系统
    this.files = Object.keys(this.assets);//['main.js']
    //存放本次编译输出的目标文件路径
    for (let file in this.assets) {
      const dir = this.options.output.path;
      let targetPath = path.join(dir, file);//page1.js page2.js
  ...
  buildModule = (name, modulePath) => {
    ...
    console.log('loader转换后的代码', targetSourceCode)
    let moduleId = './' + path.posix.relative(baseDir, modulePath);
    //webpack最核心 的几个概念要出场了 module 包括：模块ID 和 依赖的数组
    ...
    traverse(astTree, {
      CallExpression: ({ node }) => {
        if (node.callee.name === 'require') {
          ...
          console.log({ depModulePath, depModuleId }, 'ppppppppp')
          //修改抽象语法树
